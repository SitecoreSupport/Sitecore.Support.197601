...
namespace Sitecore.Form.Web.UI.Controls
{
  ...
  public class RadioList : ListControl
  {
    ...
    public override ControlResult Result
    {
      get
      {
        return new ControlResult(this.ControlName, this.buttonList.SelectedValue ?? string.Empty, (this.buttonList.SelectedItem != null) ? this.buttonList.SelectedItem.Text : string.Empty);
      }
    }
    ...
    [VisualFieldType(typeof(CssClassField)), VisualProperty("CSS Class:", 600), DefaultValue("scfRadioButtonListBorder")]
    ...
    public RadioList() : this(DependenciesManager.RequirementsChecker, HtmlTextWriterTag.Div)
    {
    }

    public RadioList(IRequirementsChecker requirementsChecker) : this(requirementsChecker, HtmlTextWriterTag.Div)
    {
    }

    public RadioList(IRequirementsChecker requirementsChecker, HtmlTextWriterTag tag) : this(requirementsChecker, tag, new ListFieldValueFormatter(DependenciesManager.Resolve<ISettings>()))
    {
    }

    public RadioList(IRequirementsChecker requirementsChecker, HtmlTextWriterTag tag, ListFieldValueFormatter listFieldValueFormatter) : base(requirementsChecker, tag, listFieldValueFormatter)
    {
